...
from transformers4token.configuration_utils import PretrainedConfig
from transformers4token.generation_utils import GenerationMixin
import torch
import torch.nn as nn
from torch.utils.data import Dataset
from typing import *
from openprompt4distilbert.data_utils import InputExample, InputFeatures
from torch.utils.data._utils.collate import default_collate
from tqdm.std import tqdm
from transformers4token.tokenization_utils import PreTrainedTokenizer
from transformers4token.utils.dummy_pt_objects import PreTrainedModel
from openprompt4distilbert.plms.utils import TokenizerWrapper
from openprompt4distilbert.prompt_base import Template, Verbalizer
from collections import defaultdict
from openprompt4distilbert.utils import round_list, signature
import numpy as np
from torch.utils.data import DataLoader
from yacs.config import CfgNode
from openprompt4distilbert.utils.logging import logger
...
class PromptForGeneration(nn.Module, GenerationMixin):
    ...
    Args:
        ...
        gen_config (:obj:`CfgNode`): The generation configs to pass into `GenerationMixin.generate <https://huggingface.co/transformers4token/_modules/transformers4token/generation_utils.html#GenerationMixin.generate>`_
        freeze_plm (:obj:`bool`): whether or not to freeze the pretrained language model
        plm_eval_mode (:obj:`bool`): this is a stronger freezing mode than freeze_plm, i.e. the dropout of the model is turned off. No matter whether the other part is set to train.
    '''
    ...
    def __init__(self,
        ...
        self.main_input_name = self.prompt_model.main_input_name # for transformers4token 4.17.0 and higher.
    ...
    def generate(self, batch: Union[Dict, InputFeatures], verbose: Optional[bool]=False, **generation_kwargs):
        ...
        ``transformers4token.generation_util.GenerationMixin.generate``
        ...
        if self.config.is_encoder_decoder:
            ...
            # Currently huggingface transformers4token only support single sample generation, or padding to the left (instead of the right).
    ...
    def prepare_inputs_for_generation(self, input_ids: Optional[torch.Tensor] = None,
                                         **model_kwargs):
        r"""This function wraps the ``prepare_inputs_for_generation`` function in the huggingface transformers4token.
